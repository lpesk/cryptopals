from message import Message, blocks, joinBlocks
...
def AES_ECB(msg, key, fn='encrypt', extra=True):
    ...
    """
    assert (len(msg) % 16 == 0), "Message length must be a multiple of 16"
    ...
    if fn == 'encrypt':
        out_blocks = [Message(cipher.encrypt(block.bytes)) for block in blocks(msg, block_size=16)]
    else:
        out_blocks = [Message(cipher.decrypt(block.bytes)) for block in blocks(msg, block_size=16)]
...
def AES_CBC(msg, key, iv='', fn='encrypt'):
    ...
    in_blocks = blocks(msg, block_size=16)
    out_blocks = []
    if fn == 'encrypt':
        out_blocks.append(iv)
        for k in range(len(in_blocks)):
            new_block = AES_ECB(XOR(in_blocks[k], out_blocks[k]), key, 'encrypt')
        ...
        for k in range(1, len(in_blocks)):
            new_block = XOR(AES_ECB(in_blocks[k], key, 'decrypt'), in_blocks[k-1])
            out_blocks.append(new_block)
    return joinBlocks(out_blocks)
